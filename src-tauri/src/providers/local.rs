//! 本地AI提供商实现

use async_trait::async_trait;
use anyhow::Result;

use super::{AIProvider, AIGenerationRequest, AIGenerationResponse, ProviderConfig, ProviderStats};

/// 本地AI提供商
pub struct LocalAIProvider {
    config: ProviderConfig,
    stats: ProviderStats,
}

impl LocalAIProvider {
    pub fn new(config: ProviderConfig) -> Self {
        Self {
            config,
            stats: ProviderStats {
                total_requests: 0,
                successful_requests: 0,
                failed_requests: 0,
                total_tokens: 0,
                total_cost: 0.0,
                avg_response_time_ms: 0.0,
                last_used: None,
            },
        }
    }
}

#[async_trait]
impl AIProvider for LocalAIProvider {
    fn name(&self) -> &str {
        "local"
    }

    fn display_name(&self) -> &str {
        "本地模型"
    }

    async fn is_available(&self) -> bool {
        // TODO: 实现可用性检查（检查本地模型文件等）
        self.config.enabled
    }

    async fn generate(&self, request: AIGenerationRequest) -> Result<AIGenerationResponse> {
        // TODO: 实现本地模型调用（如Ollama）
        Err(anyhow::anyhow!("Local AI provider not implemented"))
    }

    fn get_config(&self) -> ProviderConfig {
        self.config.clone()
    }

    async fn get_stats(&self) -> ProviderStats {
        self.stats.clone()
    }

    async fn test_connection(&self) -> Result<()> {
        // TODO: 实现连接测试（检查本地服务是否运行）
        Err(anyhow::anyhow!("Local AI connection test not implemented"))
    }
}
